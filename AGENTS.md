...
Quick summary: Next.js special files + route folders + shadcn `components/ui/*` → **kebab-case** (immutable, framework-enforced). Files we own → components `PascalCase`, hooks `useXxx.ts` camelCase, actions/services/lib/queries/validations `camelCase` or single-word lowercase. Never mix conventions in the same folder. Renaming = `git mv` + update every import + verify build (Turbopack is case-sensitive even on Windows).

**Full tables (immutable list, project convention table, forbidden patterns, renaming procedure) → `docs/agent-reference.md#4-file-naming-convention--full-tables`.**
...
Permission `(module, action)` tuples — kebab-case format, e.g. `booking:view`, `settings-users:create`, `finance-ar:edit`. **Sumber kebenaran = `moduleActions` di `prisma/seeders/roles-permissions.ts`.**
...
**Full module list (semua module + action-nya) → `docs/agent-reference.md#1-permission-module--full-table`.** Kalau butuh cek module/action valid apa aja untuk `requirePermission()`, baca file itu — jangan nebak.
...
Satu-satunya workflow aktif = `.github/workflows/ci.yml` (PR validation: generate → lint → typecheck → build). Deploy otomatis (staging/production) belum ada. Branch: `main` = staging, `production` = production, feature branch `feat/<name>`/`fix/<name>`, PR wajib ke `main`/`production`.

**Detail lengkap (deploy flow target, job structure) → `docs/agent-reference.md#3-cicd-pipeline-full`.**
...
Baca via `process.env.X`, typed di `types/env.d.ts`, jangan dibaca di components. Rate limiting pakai in-memory store (no Redis). Secrets never land in git — `.env` gitignored, `.env.example` placeholder only. Ada 24 var: DB (Neon/native pg), auth secret, S3/MinIO storage, Resend email, VAPID web-push, Peruri e-meterai, Bitrix webhook.

**Daftar lengkap + anotasi tiap var → `docs/agent-reference.md#2-env-vars-full-annotated`** (atau langsung liat `.env.example`).

---

## 12. Design System — Monochrome Chrome + Brand Accents

swasana-project = monochrome chrome (gray surfaces) + brand accents via token (`--brand-ink` `#0F4159`, `--brand-gold` `#D4A547`, `--brand-cream` `#FAF7F2`). Hardcoded hex tetep dilarang kecuali `var(--brand-*)` syntax. shadcn style base-nova (base-ui, BUKAN radix), base color neutral, icon **Solar Icons (@solar-icons/react) weight="BoldDuotone"**, font Plus Jakarta Sans (body) / Fraunces (heading) / Quicksand (logo) / Geist Mono.

**Brand palette hex table + shadcn config detail + icon usage snippet → `docs/agent-reference.md#5-design-system--full-detail`.**
...
### Layout — Bank Jago Vibe (ringkas)
Modern neobank: card-based `rounded-2xl`, button/input/badge `rounded-xl`/`rounded-full` (pill), spacing lega (`p-5`/`p-6`, `gap-4`/`gap-6`), soft shadow (`shadow-sm` → hover `shadow-md`), metric gede pakai `font-heading`, whitespace jangan sumpek. Warna tetep STRICT token-only.

**Bullet detail lengkap + icon usage code snippet → `docs/agent-reference.md#5-design-system--full-detail`.**
