    ...
    <Dialog open={open} onOpenChange={() => onClose()}>
      <DialogContent className="max-w-md">
        ...
        {loading ? (
          ...
          <p className="text-sm text-muted-foreground text-center py-4">Tidak ada data approval.</p>
        ) : (
          <div className="space-y-4">
            {/* Creator info */}
            <div className="flex items-center gap-3 p-3 rounded-lg border bg-muted/30">
              <div className="flex items-center justify-center h-7 w-7 rounded-full bg-black text-white shrink-0">
                <PenLine className="h-3.5 w-3.5" />
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium">Dibuat oleh {record.createdBy.fullName}</p>
              </div>
            </div>
              ...
              {record.steps.map((step) => (
                <div key={step.id} className={cn("flex items-center gap-3 p-3 rounded-lg border", step.status === "approved" && "bg-muted/50", step.status === "rejected" && "bg-destructive/5 border-destructive/20")}>
                  <div className={cn(
                    ...
                    {step.status === "approved" ? <Check className="h-4 w-4" /> : step.status === "rejected" ? <X className="h-4 w-4" /> : <Clock className="h-3.5 w-3.5" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    ...
                    {step.status === "approved" && step.decidedBy && (
                      <p className="text-xs text-muted-foreground">Disetujui oleh {step.decidedBy.fullName} {step.decidedAt ? `· ${new Date(step.decidedAt).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}` : ""}</p>
                    )}
                    {step.status === "rejected" && (
                      <div>
                        <p className="text-xs text-destructive">Ditolak{step.decidedBy ? ` oleh ${step.decidedBy.fullName}` : ""} {step.decidedAt ? `· ${new Date(step.decidedAt).toLocaleDateString("id-ID", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}` : ""}</p>
                        {step.notes && <p className="text-xs text-destructive/80 mt-0.5">Alasan: {step.notes}</p>}
                      </div>
                    )}
                  </div>
                </div>
            ...
            {!canApprove && activeStep && (
              <p className="text-sm text-muted-foreground text-center py-2">Menunggu approval dari {activeStep.approverType === "role" ? activeStep.approverRole?.name : activeStep.approverUser?.fullName}</p>
            )}
          </div>
        )}
      </DialogContent>
