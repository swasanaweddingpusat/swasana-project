...
export function CalendarMonthView({
  ...
  return (
    <div className={cn('overflow-hidden', 'rounded-lg', 'border', 'bg-white')}>
      <div className={cn('grid', 'grid-cols-7')}>
        {DAY_HEADERS.map((d) => (
          <div
            key={d}
            className={cn('border-b', 'border-r', 'px-2', 'py-1.5', 'text-center', 'text-xs', 'font-medium', 'text-muted-foreground', 'last:border-r-0')}
          >
            {d}
          </div>
        ))}
