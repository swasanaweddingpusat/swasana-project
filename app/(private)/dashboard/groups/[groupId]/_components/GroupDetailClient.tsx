...
export function GroupDetailClient({ group, initialPerformance, availableProfiles, currentProfileId: _currentProfileId, canManage, isSuperAdmin }: Props) {
  ...
  function handleSaveSettings() {
    startTransition(async () => {
      const res = await updateGroup({ id: group.id, name: teamName, description: teamDesc });
      if (res.success) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {isSuperAdmin && (
            <Button variant="outline" size="sm" className="gap-1.5" onClick={() => setChangeLeaderOpen(true)}>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <OverviewCard icon={Users} label="Anggota Grup" value={`${memberRows.length}`} sub="Sales aktif" />
      </div>
          ...
          <div className="flex flex-col justify-between h-full">
            <div className="space-y-4 px-2">
              <div>
                ...
                <Input className="mt-1" value={teamName} onChange={(e) => setTeamName(e.target.value)} placeholder="Nama grup" />
              </div>
              <div>
