...
interface Props { initialData: OrderStatusesResult }

const ROWS_PER_PAGE = 10;

export function OrderStatusManager({ initialData }: Props) {
  ...
  const [deleteTarget, setDeleteTarget] = useState<OrderStatusItem | null>(null);
  ...
  const handleRefresh = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json() as OrderStatusesResult;
        setItems(data);
  ...
  function handleOpenAdd() { setEditingItem(null); setFormName(""); setFormOpen(true); }
  function handleOpenEdit(item: OrderStatusItem) { setEditingItem(item); setFormName(item.name); setFormOpen(true); }

  async function handleSave() {
    if (!formName.trim()) return;
    setSaving(true);
    const result = editingItem
      ? await updateOrderStatus(editingItem.id, formName)
      : await createOrderStatus(formName);
    setSaving(false);
    ...
    if (editingItem) {
      ...
      setItems((prev) => [...prev, result.item as OrderStatusItem]);
  ...
  async function handleDelete() {
    ...
    const result = await deleteOrderStatus(deleteTarget.id);
    ...
    <>
          ...
          <CardContent className="p-0">
            ...
            <div className={cn('flex', 'flex-col', 'sm:flex-row', 'items-start', 'sm:items-center', 'justify-between', 'px-4', 'sm:px-6', 'pb-4', 'gap-3', 'border-b')}>
              <div className={cn('flex', 'items-center', 'gap-2')}>
                <h2 className={cn('text-base', 'font-bold', 'text-foreground')}>Order Status</h2>
            ...
            <div className="hidden sm:block overflow-x-auto">
              <Table>
                ...
                <TableBody>
                  {paginatedItems.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={3} className={cn('text-center', 'py-8', 'text-muted-foreground')}>Belum ada data.</TableCell>
                    </TableRow>
                  ) : (
                    paginatedItems.map((item, idx) => (
            ...
            {totalPages > 1 && (
              <PaginationBar
                ...
                label="Navigasi halaman order status"
              />
            )}
          </CardContent>
        </Card>
      ...
      <Dialog open={formOpen} onOpenChange={setFormOpen}>
        <DialogContent className="max-w-md">
          <DialogTitle>{editingItem ? "Edit" : "Tambah"} Order Status</DialogTitle>
