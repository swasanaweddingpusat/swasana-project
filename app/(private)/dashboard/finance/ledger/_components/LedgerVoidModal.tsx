...
import { MinusCircle, CloseCircle } from "@solar-icons/react";
import {
  Dialog,
  DialogContent,
  DialogTitle,
...
/** Batalkan cash-in yang sudah ter-ack (koreksi non-destruktif). */
export function LedgerVoidModal({
  open,
  onOpenChange,
  entry,
  onSuccess,
}: LedgerVoidModalProps): React.ReactElement | null {
  ...
  async function handleVoid(): Promise<void> {
    ...
    setLoading(true);
    const result = await voidCashIn({ ledgerId: entry.id, note: note.trim() });
    setLoading(false);
    if (!result.success) {
      ...
      >
        <div className="flex shrink-0 items-center justify-between gap-3 border-b border-border px-5 py-4">
          <div className="min-w-0">
            <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
              <MinusCircle weight="BoldDuotone" className="size-5 text-destructive" />
              Batalkan Transaksi
            </DialogTitle>
        ...
        <div className="flex min-h-0 flex-1 flex-col gap-4 overflow-y-auto px-5 py-5">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <Textarea
              id="void-note"
              placeholder="Mis. Pembayaran sudah dikembalikan ke client, transaksi salah input..."
              value={note}
        ...
        <div className="flex shrink-0 items-center justify-end gap-2 border-t border-border px-5 py-4">
          ...
          >
            <MinusCircle weight="BoldDuotone" className="size-4" />
            Batalkan Transaksi
          </Button>
        </div>
      </DialogContent>
