...
export async function POST(req: Request): Promise<Response> {
  const { session, response } = await requirePermissionForRoute({ module: "settings-festival", action: "create" });
  if (response) return response;
  if (!mutationLimiter.check(`festival-upload:${session.user.id}`)) return rateLimitResponse();
  ...
  try {
    ...
    const key = generateStorageKey("festivals", "webp");
    ...
    console.error("[upload/festival]", e);
    return Response.json({ error: "Gagal upload gambar festival." }, { status: 500 });
  }
}
