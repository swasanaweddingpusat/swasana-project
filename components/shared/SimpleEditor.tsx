...
import { TextBold, TextItalic, TextUnderline as UnderlineIcon, List, Checklist } from "@solar-icons/react";
...
export function SimpleEditor({ value, onChange, placeholder, className }: SimpleEditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "prose prose-sm max-w-none focus:outline-none min-h-24 px-3.5 py-3 text-sm text-foreground [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-0.5",
      },
    },
    onUpdate: ({ editor: e }) => {
      onChange(e.getHTML());
    ...
    <div className={cn("rounded-xl border border-border bg-card overflow-hidden", className)}>
      <div className="flex items-center gap-1 px-2 py-2 border-b border-border">
        ...
        <ToolbarBtn active={editor.isActive("orderedList")} onClick={() => editor.chain().focus().toggleOrderedList().run()} title="Numbered List">
          <Checklist weight="BoldDuotone" className="h-4 w-4" />
        </ToolbarBtn>
      </div>
