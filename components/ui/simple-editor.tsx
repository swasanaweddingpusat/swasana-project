...
export function SimpleEditor({ value, onChange, placeholder, className }: SimpleEditorProps) {
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: "prose prose-sm max-w-none focus:outline-none min-h-20 px-3 py-2 text-sm text-foreground [&_ul]:list-disc [&_ul]:pl-5 [&_ol]:list-decimal [&_ol]:pl-5 [&_li]:my-0.5",
      },
    },
    onUpdate: ({ editor }) => {
