...
export interface DecorationPOProps {
  booking: {
...
export function DecorationPOPdf({ booking, vendorName, poData, logoBase64, signatures }: DecorationPOProps) {
    ...
    <Document>
      <Page size="A4" style={s.page}>
        ...
        <View style={s.titleBlock}>
          <Text style={s.title}>PURCHASE ORDER DEKORASI</Text>
        ...
        <View style={s.sigBlock}>
          <View style={s.sigCol}>
            {signatures?.decoration?.finance?.signature ? (
              /* eslint-disable-next-line jsx-a11y/alt-text */
              <Image src={signatures.decoration.finance.signature} style={s.sigLine} />
            ) : (
              <View style={s.sigLine} />
            )}
            <Text style={s.sigName}>{signatures?.decoration?.finance?.name ?? ""}</Text>
            <Text style={s.sigTitle}>Finance</Text>
          </View>
          <View style={s.sigCol}>
            {signatures?.decoration?.dirops?.signature ? (
              /* eslint-disable-next-line jsx-a11y/alt-text */
              <Image src={signatures.decoration.dirops.signature} style={s.sigLine} />
            ) : (
              <View style={s.sigLine} />
            )}
            <Text style={s.sigName}>{signatures?.decoration?.dirops?.name ?? ""}</Text>
            <Text style={s.sigTitle}>Direktur Ops</Text>
          </View>
          <View style={s.sigCol}>
            {signatures?.decoration?.operations?.signature ? (
              /* eslint-disable-next-line jsx-a11y/alt-text */
              <Image src={signatures.decoration.operations.signature} style={s.sigLine} />
            ) : (
              <View style={s.sigLine} />
            )}
            <Text style={s.sigName}>{signatures?.decoration?.operations?.name ?? ""}</Text>
