...
export async function bulkCreateWorkAssignment(data: unknown): Promise<{ success: boolean; error?: string }> {
  ...
  const { profileIds, workLocationId, workShiftId, isDefault, offdayDays, effectiveDate, endDate, isMobileAttendance, clockInLocationId, clockOutLocationId } = parsed.data;

  try {
    await db.$transaction(
      profileIds.map((profileId) =>
        db.employeeWorkAssignment.create({
          data: { profileId, workLocationId, workShiftId, isDefault, offdayDays, effectiveDate, endDate, isMobileAttendance, clockInLocationId, clockOutLocationId },
        })
      )
    );
